...
# back-transform sqrt & convert kg -> g
data.frame(Tukey_fit3$Substrate[,1:3] ^ 2 * 1000)
```
...
data.frame(
  Estimate = (summary(fit3)$coefficients[,1]
              [summary(fit3)$coefficients[,4] < 0.05]) ^ 2,
  Conf.Int = (confint(fit3) 
              [summary(fit3)$coefficients[,4] < 0.05,]) ^ 2
)
```
