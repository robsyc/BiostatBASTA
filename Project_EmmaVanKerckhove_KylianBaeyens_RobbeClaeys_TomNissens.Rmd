...
```{r}
data <- read.delim(
  "data/BASTA_Dataset2_Trichoderma.txt", 
...
At first sight, we can see that substrate choice (and to a lesser extent *Trichoderma* addition) on average seem to have a effect on the weight of curved strawberries. The choice of greenhouse, however, does not seem to have an effect, but a slightly higher variance can be observed for greenhouse 1. There seems to be no equality of variances (heteroscedasticity) between the 16 different combinations upon viewing the height of the barcharts from any of the frames generated.
...
A square root transformation has a beneficial effect on the normality of the data, the departures from normality are also symmetric. 
Back-transformation will have to be performed during model interpretation. It was noticed however, that there is an observation where the weight of curved strawberries is zero. 
This value doesn't hinder further analysis and is still taken into account.

Although homoscedasticity seems to be met (based on the previously shown box plots), an additional sanity check is performed with Levene’s test (which is robust against normality violations). 
If the resulting p-value is greater than 0.05, the null-hypothesis cannot be rejected - meaning that sample variance is likely to have occurred based on random sampling from a population with equal variances.

```{r warning=FALSE}
leveneTest(
  Curved_strawberries ~ interaction(Substrate, Trichoderma), 
  data = df_t)
```
...
fit1 = lm(
  Curved_strawberries ~ Substrate + Trichoderma + Substrate:Trichoderma, 
  data = df_t)
...
The assumptions and testing plots show good results but no 5% significance is obtained for the overall model (p-value: 0.07216). 
Analysis of variance (ANOVA) is performed to check which variables best account for the variance present in the data.
...
Additionaly, the variance inflation factor (VIF) for the fit can indicate whether there is multicollinearity between two or more variables.

```{r, warning=FALSE}
vif(fit1, type = "predictor")
```

The GVIF values (generalized VIF) indicate there is no correlation between the predictor variables in the model, therefore, all variables of fit1 are kept in the final model. Though a more simple model will also be investigated:

Linear model fit with only substrate as explanatory variable:

```{r}
fit2 <- lm(
  Curved_strawberries ~ Substrate, 
  data = df_t)
...
```{r}
fit3 = lm(
  Curved_strawberries ~ Substrate + Trichoderma + Greenhouse + Substrate:Trichoderma, 
  data = df_t)
...
The model produces less significant results when incorporating the greenhouse choice as a parameter (p-value: 0.09714 vs. 0.07216), but it is kept in the model in order to account for possible technical variation. Samples with Substrate:Peat and/or *Trichoderma*:Trich3 generate the most significant results, notably, the combination of both these variables shows a large difference in weight (before back-transformation).
...
The GVIF values are close to 1, indicating that there is no alarming correlation between any explanatory variables after adding the greenhouse variable.
...
When performing an ANOVA test, it can be observed that there is only a significant difference when comparing different substrates. A Tukey post-hoc analysis is performed to compare the different substrates. 
...
It can be concluded that only a significant difference in weight of curved strawberries can be found when comparing the substrates *peatfree-biochar* & *peat-biochar* (p-value = 0,021). 

The results are back-transformed for interpretation on the original scale.

For the Substrate Tukey-test:

```{r}
# Peatfree_Biochar-Peat_Biochar
# back-transform sqrt & convert kg -> g & sign
data.frame(
  abs(Tukey_fit3$Substrate[,1:3]) ^ 2 * 1000 * sign(Tukey_fit3$Substrate[,1:3])
  )
```

For the significant variables in the overall fit:

```{r}
dim(summary(fit3)$coefficients)
# columns: Estimate Std. Error    t value     Pr(>|t|)
data.frame(
  Estimate = abs(summary(fit3)$coefficients[,1]
              [summary(fit3)$coefficients[,4] < 0.05]) ^ 2 * 1000 * sign(
                (summary(fit3)$coefficients[,1]
                 [summary(fit3)$coefficients[,4] < 0.05])),
  
  Conf.Int = abs(confint(fit3) 
              [summary(fit3)$coefficients[,4] < 0.05,]) ^ 2 * 1000 * sign(
                (confint(fit3) 
                 [summary(fit3)$coefficients[,4] < 0.05,])
              )
...
Some of the variable (combinations) show to have an effect on the weight of curved strawberries, especially the Peat:Trich3 interaction, though the confidence intervals are very broad.
...
The power of the linear model is 69%. This indicates that there is a 69% probability of not making a type 2 error. This could be improved by repeating the experiment with more samples. Next, we determine the amount of samples that should be taken to improve power to 80%.
...
No lineair model was found that showed significant results when accounting for the greenhouse choice. The assumptions to use a lineair model were met (normality, homoscedascity, linearity). A linear model could be made with substrate-choice, *trichoderma*-addition, greenhouse blocking and a substrate-*trichoderma* interaction as explanatory variables. Although the model was not significant (p = 0.09714) further statistical testing was performed. The variance inflation factor indicated no correlation between any of the explanatory variables when greenhouse blocking was applied.

The ANOVA test indicated that there are only significant differences in the weight of curved strawberries between different substrates. Further analysis with a Tukey post-hoc test compared the effect between the different substrates. The analysis indicated that a significant difference in the weight of the curved strawberries can be found when comparing peatfree substrate with biochar and substrate containing peat and biochar (p = 0.0210655). The difference in average weight of the curved strawberries per plant when adding peat to the biochar substrate is 1.51184354 g (with a 95% CI in the range [0.02009253, 5.3703094]).

The lack of significant findings was likely due to a lack of power. A power analysis indicated that our linear model only had a power of 69%, this could be improved by repeating the experiment with more samples. The power analysis also indicated that the power of our model could be increased to 80% by taking at least 80 samples (instead of 64).