...
At first sight, we can see that substrate choice (and to a lesser extent *Trichoderma* addition) on average seem to have an effect on the weight of curved strawberries. The choice of greenhouse, however, does not seem to have an effect, but a slightly higher variance can be observed for greenhouse 1. There seems to be no equality of variances (heteroscedasticity) between the 16 different combinations upon viewing the height of the barcharts from any of the frames generated.
...
When performing an ANOVA test, it can be observed that there is only a significant difference between the different substrates. A Tukey post-hoc analysis is performed to compare the different substrates. 
...
It can be concluded that only a significant difference in weight of curved strawberries can be found when comparing the substrates peatfree with biochar & peat with biochar (p-value = 0,021). 
