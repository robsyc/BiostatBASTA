...
```{r}
fit3 = lm(Curved_strawberries ~ Substrate + Greenhouse, data = df_t)
summary(fit3)
```

The model produces slightly less significant results when incorporating the greenhouse choice as a parameter, but it is kept in the model in order to account for possible technical variation.

Further statistical testing and pairwise comparison is performed.

```{r}
anova(fit3)
fit3.mcp <- glht(fit3, linfct = mcp(Substrate = "Tukey"))
summary(fit3.mcp)
confint(fit3.mcp)
```
