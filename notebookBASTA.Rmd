...
We can see that substrate choice (and to a lesser extent *Trichoderma* addition) seems to have an effect on the weight of curved strawberries. The choice of greenhouse, however, does not seem to have an effect but a slightly higher variance can be observed for greenhouse 1. There seems to be no equality of variances between the 16 different combinations upon viewing the height of the barcharts from any of the frames generated.
...
An overall p-value of 0.0442 for the model is obtained, indicating that substrate is indeed the primary driver of curved strawberry weight differences. Additionaly, the variance inflation factor (VIF) for the first fit can indicate whether there is multicollinearity between two or more variables.
...
tukey1 = glht(fit3, linfct = mcp(Substrate = "Tukey"))
tukey1
```

Due to the presence of covariate interactions, a post hoc Tukey test can't be performed. Instead, we use a matrix model to study the effect on the weight of the curved strawberries between the used substrates.
