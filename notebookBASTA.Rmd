...
We can see that substrate choice (and to a lesser extent *Trichoderma* addition) seems to have an effect on the weight of curved strawberries. The choice of greenhouse, however, does not seem to have an effect but does show difference in variance.
...
A square root transformation had a beneficial effect on the normality of the data. Backtranstransformation will have to be performed during model interpretation. *It was noticed however, that there is an observation where the weight of curved strawberries is zero. ...*

Although homoscedasticity seems to be met (based on the previous boxplots), an additional sanity check is performed with Levene’s test (robust against normality violations). If the resulting p-value is greater than 0.05, the null-hypothesis cannot be rejected - meaning that sample variance is likely to have occurred based on random sampling from a population with equal variances.
...
The assumptions and testing plots show good results but no 5% significance is obtained (p-value: 0.07216).
...
When performing anova, only a significant result can be obtained for the substrate as explanatory variable.
...
The greenhouse choice could be responsible for technical variation on the data. As the greenhouses can have a slightly different temperature, humidity, ... Therefore the greenhouse variable is incorporated in the model. (blocking)

```{r}
fit3 = lm(Curved_strawberries ~ Substrate + Trichoderma + Substrate:Trichoderma + Greenhouse, data = df_t)
summary(fit3)
plot(fit3)
```

The model produces less significant results when incorporating the greenhouse choice as a parameter, but it is kept in the model in order to account for possible technical variation.

Further statistical testing and pairwise comparison is performed.
...
# pairwise comparisons against the control for H1 (2 methods)
H1 <- msqrob2::makeContrast(c("SubstratePeat = 0", "SubstratePeatfree_Biochar= 0", "SubstratePeat_Biochar = 0"), parameterNames = colnames(X))
testH1 <- glht(fit, linfct = t(H1))
summary(testH1)
confint(testH1)
plot(testH1)

print("H01: On average, the growing medium has no effect on the weight of the curved strawberries:")
fit3S.mcp <- glht(fit3, linfct = mcp(Substrate = "Tukey"))
summary(fit3S.mcp)
confint(fit3S.mcp)
plot(fit3S.mcp)
```
...
```{r}
# pairwise comparisons against the control for H2 (2 methods)
H2 <- msqrob2::makeContrast(c("TrichodermaTrich1 = 0", "TrichodermaTrich2= 0", "TrichodermaTrich3 = 0"), parameterNames = colnames(X))
testH2 <- glht(fit, linfct = t(H2))
summary(testH2)
confint(testH2)
plot(testH2)

print("H02: On average, the addition of Trichoderma has no effect on the weight of the curved strawberries:")
fit3T.mcp <- glht(fit3, linfct = mcp(Trichoderma = "Tukey"))
summary(fit3T.mcp)
confint(fit3T.mcp)
plot(fit3T.mcp)
```
...
```{r}
# pairwise comparisons against the control for H3 (1 method)
H3 <- msqrob2::makeContrast(c("SubstratePeat:TrichodermaTrich1 = 0", "SubstratePeat_Biochar:TrichodermaTrich1 = 0",
                              "SubstratePeatfree_Biochar:TrichodermaTrich1 = 0", "SubstratePeat:TrichodermaTrich2 = 0", 
                              "SubstratePeat_Biochar:TrichodermaTrich2 = 0", "SubstratePeatfree_Biochar:TrichodermaTrich2 = 0", 
                              "SubstratePeat:TrichodermaTrich3 = 0", "SubstratePeat_Biochar:TrichodermaTrich3 = 0", 
                              "SubstratePeatfree_Biochar:TrichodermaTrich3 = 0"), parameterNames = colnames(X))
testH3 <- glht(fit, linfct = t(H3))
summary(testH3)
confint(testH3)
plot(testH3)
```
...
A significant result is only obtained for the pairwise comparison between biochar & biochar-peat substrates (p-value: 0.0259).
